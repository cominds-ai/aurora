...
class BaseAgent(ABC):
    ...
    async def _run_conversation(
        ...
        message = await self._invoke_llm(initial_messages, format)
    ...
    async def invoke(self, query: str, format: Optional[str] = None) -> AsyncGenerator[BaseEvent, None]:
        ...
        async for event in self._run_conversation([{"role": "user", "content": query}], format):
            yield event

    async def invoke_with_attachments(
            self,
            query: str,
        ...
        format = format if format else self._format
        if not attachments:
            async for event in self.invoke(query, format):
                yield event
            return

        content: List[Dict[str, Any]] = [{"type": "text", "text": query}]
        for attachment in attachments:
            if attachment.mime_type.startswith("image/") and attachment.url:
                content.append({"type": "image_url", "image_url": {"url": attachment.url}})

        async for event in self._run_conversation([{"role": "user", "content": content}], format):
            yield event
