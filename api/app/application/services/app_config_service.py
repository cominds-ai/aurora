...
class AppConfigService:
    ...
    async def _load_app_config(self) -> AppConfig:
        async with self._uow_factory() as uow:
            return await uow.user_config.load(self._user_id)

    async def _save_app_config(self, app_config: AppConfig) -> None:
        async with self._uow_factory() as uow:
            await uow.user_config.save(self._user_id, app_config)
    ...
    async def get_search_config(self) -> SearchConfig:
        app_config = await self._load_app_config()
    ...
    async def get_sandbox_preference(self) -> SandboxPreference:
        app_config = await self._load_app_config()
        return app_config.sandbox_preference

    async def update_sandbox_preference(self, sandbox_preference: SandboxPreference) -> SandboxPreference:
        app_config = await self._load_app_config()
        app_config.sandbox_preference = sandbox_preference
        await self._save_app_config(app_config)
        return app_config.sandbox_preference
