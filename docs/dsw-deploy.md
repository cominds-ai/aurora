...
在 `aurora-sandbox-*` 实例中，建议关闭沙箱进程自身的自动销毁定时器，避免实例运行正常但 `8080/9222/5901` 在空闲约 60 分钟后被 sandbox 自己关闭：

- `SANDBOX_TIMEOUT_MINUTES=0`
...
OSS_SCHEME=https
AURORA_OFFICIAL_DEFAULT_GEMINI3_API_KEY=官方默认gemini3
AURORA_OFFICIAL_DEFAULT_CLAUDE_API_KEY=官方默认claude
EOF
```
